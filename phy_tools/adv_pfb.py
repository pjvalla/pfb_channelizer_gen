...
def gen_exp_shift_rtl(path, chan_obj, cic_obj):
    ...
    with open(file_name, 'w') as fh:
        ...
        fh.write('\n')
        fh.write('reg [{}:0] tdatai[0:3];\n'.format(iw - 1))
        fh.write('reg [{}:0] tdataq[0:3];\n'.format(iw - 1))
        fh.write('\n')
        fh.write('reg [{}:0] tuser_d[0:4];\n'.format(tuser_bits - 1))
        fh.write('reg [4:0] tlast_d;\n')
        fh.write('reg [4:0] take_d;\n')
        fh.write('\n')
        ...
        fh.write('assign fft_bin = tuser_d[1][{}:0];\n'.format(fft_msb))
        ...
        fh.write('\n')
        fh.write('assign chan_0 = (fft_bin == {}\'d0 && take_d[1] == 1\'b1) ? 1\'b1 : 1\'b0;\n'.format(fft_msb + 1))
        fh.write('\n')
        ridx = idx_bytes * 8
        lidx = ridx + 4
        ...
        fh.write('assign curr_shift = tuser_d[1][{}:{}];\n'.format(lidx, ridx))
        fh.write('\n')
        pad_size = (48 - iw) // 2
        fh.write(
            'assign pcorr_i = {{ {{{}{{tdatai[3][{}]}}}}, tdatai[3], {{{{{}{{tdatai[3][0]}}}}}} }};\n'.format(pad_size, iw-1, pad_size))
        fh.write(
            'assign pcorr_q = {{ {{{}{{tdataq[3][{}]}}}}, tdataq[3], {{{{{}{{tdataq[3][0]}}}}}} }};\n'.format(pad_size, iw-1, pad_size))
        fh.write('\n')
        fh.write('\n')
        ...
        fh.write('    .clk(clk),\n')
        fh.write('    .sync_reset(sync_reset),\n')
        fh.write('    .s_axis_tvalid(take_d[4]),\n')
        fh.write('    .s_axis_tdata(fifo_tdata),\n')
        fh.write('    .s_axis_tlast(tlast_d[4]),\n')
        fh.write('    .s_axis_tuser(tuser_d[4]),\n')
        fh.write('    .s_axis_tready(),\n')
        fh.write('    .almost_full(almost_full),\n')
        fh.write('    .m_axis_tvalid(m_axis_tvalid),\n')
        fh.write('    .m_axis_tdata(m_axis_tdata),\n')
