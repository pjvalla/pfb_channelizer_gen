    ...
    def lms(self, frames, train_symbols, num_taps=5, mu=.005):
        ...
        for nn, in_vec in enumerate(frames):
            ...
            if self.plot_on:
                ...
                fig.canvas.set_window_title('Equalized Burst')
                fig.canvas.draw()
                fig.savefig('../figures/' + title + '.png')
    ...
    def fitz_freq_corr(self, bursts, num_terms=10):
        ...
        for ii, burst in enumerate(bursts):
            ...
            if self.plot_on:
                ...
                ax2.set_title('Q component')
                fig.canvas.set_window_title('Frequency/Phase Corrected Burst')
                fig.canvas.draw()
                fig.savefig('../figures/' + title + '.png')
    ...
    def decode(self, in_vec, on_factor=6, use_fll=False):
        ...
        for start_idx in frame_idx:
            ...
            if use_fll:
                ...
                ax2.set_title('Q component')
                fig.canvas.set_window_title('Before FLL Burst')
                fig.canvas.draw()
                fig.savefig('../figures/' + title + '.png')
...
def gen_input():
    ...
    fig.canvas.set_window_title('Input Signal')
    fig.canvas.draw()
